...
enum class TrxValidationStatus {
    ...
    VoteThreshold    /** If not enough peernodes have voted on a finalized budget */
};
...
//
// CBudgetVote - Allow a peernode node to vote and broadcast throughout the network
//
...
//
// CFinalizedBudgetVote - Allow a peernode node to vote and broadcast throughout the network
//
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (peernode only)
...
public:
    ...
    //vote on this finalized budget as a peernode
    void SubmitVote();
...
//
// Budget Proposal : Contains the peernode votes for each budget
//
