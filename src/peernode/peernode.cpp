...
//
// When a new peernode broadcast is sent, update our information
//
...
uint256 CMasternode::CalculateScore(int mod, int64_t nBlockHeight)
{
    ...
    if (!GetBlockHash(hash, nBlockHeight)) {
        LogPrint("peernode","CalculateScore ERROR - nHeight %d - Returned 0\n", nBlockHeight);
        return 0;
    }
...
int64_t CMasternode::GetLastPaid()
{
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
        ...
        if (peernodePayments.mapMasternodeBlocks.count(BlockReading->nHeight)) {
            /*
                Search for this payee, with at least 2 votes. This will aid in consensus allowing the network
                to converge on the same payees quickly, then keep the same schedule.
            */
            if (peernodePayments.mapMasternodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2)) {
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, bool fOffline)
{
    ...
    if (!fOffline && !peernodeSync.IsBlockchainSynced()) {
        strErrorRet = "Sync in progress. Must wait until sync is complete to start Masternode";
        LogPrint("peernode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }

    if (!obfuScationSigner.GetKeysFromSecret(strKeyMasternode, keyMasternodeNew, pubKeyMasternodeNew)) {
        strErrorRet = strprintf("Invalid peernode key %s", strKeyMasternode);
        LogPrint("peernode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }

    if (!pwalletMain->GetMasternodeVinAndKeys(txin, pubKeyCollateralAddressNew, keyCollateralAddressNew, strTxHash, strOutputIndex)) {
        strErrorRet = strprintf("Could not allocate txin %s:%s for peernode %s", strTxHash, strOutputIndex, strService);
        LogPrint("peernode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }
...
bool CMasternodeBroadcast::Create(CTxIn txin, CService service, CKey keyCollateralAddressNew, CPubKey pubKeyCollateralAddressNew, CKey keyMasternodeNew, CPubKey pubKeyMasternodeNew, std::string& strErrorRet, CMasternodeBroadcast& mnbRet)
{
    ...
    LogPrint("peernode", "CMasternodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyMasternodeNew.GetID() = %s\n",
        CBitcoinAddress(pubKeyCollateralAddressNew.GetID()).ToString(),
        pubKeyMasternodeNew.GetID().ToString());

    CMasternodePing mnp(txin);
    if (!mnp.Sign(keyMasternodeNew, pubKeyMasternodeNew)) {
        strErrorRet = strprintf("Failed to sign ping, peernode=%s", txin.prevout.hash.ToString());
        LogPrint("peernode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
    ...
    if (!mnbRet.IsValidNetAddr()) {
        strErrorRet = strprintf("Invalid IP address %s, peernode=%s", mnbRet.addr.ToStringIP (), txin.prevout.hash.ToString());
        LogPrint("peernode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        mnbRet = CMasternodeBroadcast();
        return false;
    }
    ...
    if (!mnbRet.Sign(keyCollateralAddressNew)) {
        strErrorRet = strprintf("Failed to sign broadcast, peernode=%s", txin.prevout.hash.ToString());
        LogPrint("peernode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        mnbRet = CMasternodeBroadcast();
        return false;
    }
...
bool CMasternodeBroadcast::CheckDefaultPort(std::string strService, std::string& strErrorRet, std::string strContext)
{
    ...
    if (service.GetPort() != nDefaultPort) {
        strErrorRet = strprintf("Invalid port %u for peernode %s, only %d is supported on %s-net.",
                                        service.GetPort(), strService, nDefaultPort, Params().NetworkIDString());
        LogPrint("peernode", "%s - %s\n", strContext, strErrorRet);
        return false;
    }
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrint("peernode","mnb - Signature rejected, too far into the future %s\n", vin.prevout.hash.ToString());
        nDos = 1;
        return false;
    }
    ...
    if (protocolVersion < peernodePayments.GetMinMasternodePaymentsProto()) {
        LogPrint("peernode","mnb - ignoring outdated Masternode %s protocol version %d\n", vin.prevout.hash.ToString(), protocolVersion);
        return false;
    }
    ...
    if (pubkeyScript.size() != 25) {
        LogPrint("peernode","mnb - pubkey the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        LogPrint("peernode","mnb - pubkey2 the wrong size\n");
        nDos = 100;
        return false;
    }

    if (!vin.scriptSig.empty()) {
        LogPrint("peernode","mnb - Ignore Not Empty ScriptSig %s\n", vin.prevout.hash.ToString());
        return false;
    }
    ...
    {
        // don't ban for old peernodes, their sigs could be broken because of the bug
    ...
    // no such peernode, nothing to update
    ...
    // peernode is not enabled yet/already, nothing to update
    ...
    if (pmn->pubKeyCollateralAddress == pubKeyCollateralAddress && !pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS)) {
        ...
        LogPrint("peernode","mnb - Got updated entry for %s\n", vin.prevout.hash.ToString());
        if (pmn->UpdateFromNewBroadcast((*this))) {
            pmn->Check();
            if (pmn->IsEnabled()) Relay();
        }
        peernodeSync.AddedMasternodeList(GetHash());
    }
...
{
    // we are a peernode with the same vin (i.e. already activated) and this mnb is ours (matches our Masternode privkey)
    ...
    if (pmn != NULL) {
        // nothing to do here if we already know about this peernode and it's enabled
        if (pmn->IsEnabled()) return true;
...

    {
        ...
        if (!lockMain) {
            ...
            peernodeSync.mapSeenSyncMNB.erase(GetHash());
            return false;
        }
    ...
    LogPrint("peernode", "mnb - Accepted Masternode entry\n");

    if (GetInputAge(vin) < MASTERNODE_MIN_CONFIRMATIONS) {
        LogPrint("peernode","mnb - Input must have at least %d confirmations\n", MASTERNODE_MIN_CONFIRMATIONS);
    ...
    LogPrint("peernode","mnb - Got NEW Masternode entry - %s - %lli \n", vin.prevout.hash.ToString(), sigTime);
    CMasternode mn(*this);
    mnodeman.Add(mn);
...
{
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrint("peernode","CMasternodePing::CheckAndUpdate - Signature rejected, too far into the future %s\n", vin.prevout.hash.ToString());
        nDos = 1;
        return false;
    }

    if (sigTime <= GetAdjustedTime() - 60 * 60) {
        LogPrint("peernode","CMasternodePing::CheckAndUpdate - Signature rejected, too far into the past %s - %d %d \n", vin.prevout.hash.ToString(), sigTime, GetAdjustedTime());
        nDos = 1;
        return false;
    }
    ...
    LogPrint("peernode", "CMasternodePing::CheckAndUpdate - New Ping - %s - %s - %lli\n", GetHash().ToString(), blockHash.ToString(), sigTime);
    ...
    if (pmn != NULL && pmn->protocolVersion >= peernodePayments.GetMinMasternodePaymentsProto()) {
        ...
        // LogPrint("peernode","mnping - Found corresponding mn for vin: %s\n", vin.ToString());
        // update only if there is no known ping for this peernode or
        ...
        if (!pmn->IsPingedWithin(MASTERNODE_MIN_MNP_SECONDS - 60, sigTime)) {
            ...
            if (mi != mapBlockIndex.end() && (*mi).second) {
                if ((*mi).second->nHeight < chainActive.Height() - 24) {
                    LogPrint("peernode","CMasternodePing::CheckAndUpdate - Masternode %s block hash %s is too old\n", vin.prevout.hash.ToString(), blockHash.ToString());
                ...
                if (fDebug) LogPrint("peernode","CMasternodePing::CheckAndUpdate - Masternode %s block hash %s is unknown\n", vin.prevout.hash.ToString(), blockHash.ToString());
            ...
            LogPrint("peernode", "CMasternodePing::CheckAndUpdate - Masternode ping accepted, vin: %s\n", vin.prevout.hash.ToString());

            Relay();
            return true;
        }
        LogPrint("peernode", "CMasternodePing::CheckAndUpdate - Masternode ping arrived too early, vin: %s\n", vin.prevout.hash.ToString());
        //nDos = 1; //disable, this is happening frequently and causing banned peers
        return false;
    }
    LogPrint("peernode", "CMasternodePing::CheckAndUpdate - Couldn't find compatible Masternode entry, vin: %s\n", vin.prevout.hash.ToString());
