...
bool CMasternodeDB::Write(const CMasternodeMan& mnodemanToSave)
{
    ...
    ssMasternodes << strMagicMessage;                   // peernode cache file specific magic message
    ...
    LogPrint("peernode","Written info to mncache.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodeDB::ReadResult CMasternodeDB::Read(CMasternodeMan& mnodemanToLoad, bool fDryRun)
{
    ...
    try {
        ...
        if (strMagicMessage != strMagicMessageTmp) {
            error("%s : Invalid peernode cache magic message", __func__);
    ...
    LogPrint("peernode","Loaded info from mncache.dat  %dms\n", GetTimeMillis() - nStart);
    LogPrint("peernode","  %s\n", mnodemanToLoad.ToString());
    if (!fDryRun) {
        LogPrint("peernode","Masternode manager - cleaning....\n");
        mnodemanToLoad.CheckAndRemove(true);
        LogPrint("peernode","Masternode manager - result:\n");
        LogPrint("peernode","  %s\n", mnodemanToLoad.ToString());
    }
...
void DumpMasternodes()
{
    ...
    LogPrint("peernode","Verifying mncache.dat format...\n");
    CMasternodeDB::ReadResult readResult = mndb.Read(tempMnodeman, true);
    // there was an error and it was not an error on file opening => do not proceed
    if (readResult == CMasternodeDB::FileError)
        LogPrint("peernode","Missing peernode cache file - mncache.dat, will try to recreate\n");
    else if (readResult != CMasternodeDB::Ok) {
        LogPrint("peernode","Error reading mncache.dat: ");
    ...
    LogPrint("peernode","Writting info to mncache.dat...\n");
    mndb.Write(mnodeman);

    LogPrint("peernode","Masternode dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
bool CMasternodeMan::Add(CMasternode& mn)
{
    ...
    if (pmn == NULL) {
        LogPrint("peernode", "CMasternodeMan: Adding new Masternode %s - %i now\n", mn.vin.prevout.hash.ToString(), size() + 1);
...
void CMasternodeMan::AskForMN(CNode* pnode, CTxIn& vin)
{
    ...
    LogPrint("peernode", "CMasternodeMan::AskForMN - Asking node for missing entry, vin: %s\n", vin.prevout.hash.ToString());
...
void CMasternodeMan::CheckAndRemove(bool forceExpiredRemoval)
{
    ...
    while (it != vMasternodes.end()) {
        if ((*it).activeState == CMasternode::MASTERNODE_REMOVE ||
            ...
            (*it).protocolVersion < peernodePayments.GetMinMasternodePaymentsProto()) {
            LogPrint("peernode", "CMasternodeMan: Removing inactive Masternode %s - %i now\n", (*it).vin.prevout.hash.ToString(), size() - 1);
            ...
            while (it3 != mapSeenMasternodeBroadcast.end()) {
                if ((*it3).second.vin == (*it).vin) {
                    peernodeSync.mapSeenSyncMNB.erase((*it3).first);
            ...
            // allow us to ask for this peernode again if we see another ping
    ...
    while (it3 != mapSeenMasternodeBroadcast.end()) {
        if ((*it3).second.lastPing.sigTime < GetTime() - (MASTERNODE_REMOVAL_SECONDS * 2)) {
            mapSeenMasternodeBroadcast.erase(it3++);
            peernodeSync.mapSeenSyncMNB.erase((*it3).second.GetHash());
        } else {
            ++it3;
        }
    }
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        ...
        if (IsSporkActive (SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
            ...
            if ((nMasternode_Age) < nMasternode_Min_Age) {
                continue; // Skip peernodes younger than (default) 8000 sec (MUST be > MASTERNODE_REMOVAL_SECONDS)
            }
        }
        mn.Check ();
        if (!mn.IsEnabled ())
            continue; // Skip not-enabled peernodes
...
{
    int i = 0;
    protocolVersion = protocolVersion == -1 ? peernodePayments.GetMinMasternodePaymentsProto() : protocolVersion;
        ...
        if (!(pnode->addr.IsRFC1918() || pnode->addr.IsLocal())) {
            ...
            if (it != mWeAskedForMasternodeList.end()) {
                if (GetTime() < (*it).second) {
                    LogPrint("peernode", "dseg - we already asked peer %i for the list; skipping...\n", pnode->GetId());
                    return;
                }
            }
        }
...
//
// Deterministically select the oldest/best peernode to pay on the network
//
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCount)
{
    ...
    */
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        ...
        if (mn.protocolVersion < peernodePayments.GetMinMasternodePaymentsProto()) continue;

        //it's in the list (up to 8 entries ahead of current block to allow propagation) -- so let's skip it
        if (peernodePayments.IsScheduled(mn, nBlockHeight)) continue;

        //it's too new, wait for a cycle
        if (fFilterSigTime && mn.sigTime + (nMnCount * 2.6 * 60) > GetAdjustedTime()) continue;

        //make sure it has as many confirmations as there are peernodes
...
CMasternode* CMasternodeMan::FindRandomNotInVec(std::vector<CTxIn>& vecToExclude, int protocolVersion)
{
    ...
    LogPrint("peernode", "CMasternodeMan::FindRandomNotInVec - nCountEnabled - vecToExclude.size() %d\n", nCountEnabled - vecToExclude.size());
    if (nCountEnabled - vecToExclude.size() < 1) return NULL;

    int rand = GetRandInt(nCountEnabled - vecToExclude.size());
    LogPrint("peernode", "CMasternodeMan::FindRandomNotInVec - rand %d\n", rand);
    bool found;
...
int CMasternodeMan::GetMasternodeRank(const CTxIn& vin, int64_t nBlockHeight, int minProtocol, bool fOnlyActive)
{
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        if (mn.protocolVersion < minProtocol) {
            LogPrint("peernode","Skipping Masternode with obsolete version %d\n", mn.protocolVersion);
            continue;                                                       // Skip obsolete versions
        }
        ...
        if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
            nMasternode_Age = GetAdjustedTime() - mn.sigTime;
            if ((nMasternode_Age) < nMasternode_Min_Age) {
                if (fDebug) LogPrint("peernode","Skipping just activated Masternode. Age: %ld\n", nMasternode_Age);
                continue;                                                   // Skip peernodes younger than (default) 1 hour
            }
        }
        if (fOnlyActive) {
            mn.Check();
...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        if (pnode->fObfuScationMaster) {
            ...
            LogPrint("peernode","Closing Masternode connection peer=%i \n", pnode->GetId());
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (mapSeenMasternodeBroadcast.count(mnb.GetHash())) { //seen
            peernodeSync.AddedMasternodeList(mnb.GetHash());
            return;
        }
        ...
        if (mnb.CheckInputsAndAdd(nDoS)) {
            ...
            LogPrint("peernode","mnb - Rejected Masternode entry %s\n", mnb.vin.prevout.hash.ToString());
        ...
        LogPrint("peernode", "mnp - Masternode ping, vin: %s\n", mnp.vin.prevout.hash.ToString());
        ...
        // we might have to ask for a peernode entry once
        ...
        BOOST_FOREACH (CMasternode& mn, vMasternodes) {
            ...
            if (mn.IsEnabled()) {
                LogPrint("peernode", "dseg - Sending Masternode entry - %s \n", mn.vin.prevout.hash.ToString());
                if (vin == CTxIn() || vin == mn.vin) {
                    ...
                    if (vin == mn.vin) {
                        LogPrint("peernode", "dseg - Sent 1 Masternode entry to peer %i\n", pfrom->GetId());
                        return;
                    }
                }
            }
        ...
        if (vin == CTxIn()) {
            ...
            LogPrint("peernode", "dseg - Sent %d Masternode entries to peer %i\n", nInvCount, pfrom->GetId());
        }
    }
    /*
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (protocolVersion < peernodePayments.GetMinMasternodePaymentsProto()) {
            LogPrintf("CMasternodeMan::ProcessMessage() : dsee - ignoring outdated Masternode %s protocol version %d < %d\n", vin.prevout.hash.ToString(), protocolVersion, peernodePayments.GetMinMasternodePaymentsProto());
            Misbehaving(pfrom->GetId(), 1);
            return;
        }
        ...
        if (pmn != NULL) {
            ...
            if (count == -1 && pmn->pubKeyCollateralAddress == pubkey && (GetAdjustedTime() - pmn->nLastDsee > MASTERNODE_MIN_MNB_SECONDS)) {
                ...
                if (pmn->nLastDsee < sigTime) { //take the newest entry
                    LogPrint("peernode", "dsee - Got updated entry for %s\n", vin.prevout.hash.ToString());
                    ...
                    if (pmn->IsEnabled()) {
                        ...
                        if (!lockNodes) return;
                        BOOST_FOREACH (CNode* pnode, vNodes)
                            if (pnode->nVersion >= peernodePayments.GetMinMasternodePaymentsProto())
        ...
        if (mapSeenDsee.count(vin.prevout) && mapSeenDsee[vin.prevout] == pubkey) {
            LogPrint("peernode", "dsee - already seen this vin %s\n", vin.prevout.ToString());
            return;
        }
        ...
        LogPrint("peernode", "dsee - Got NEW OLD Masternode entry %s\n", vin.prevout.hash.ToString());
        ...
        if CTxOut vout = CTxOut((MASTERNODE_COLLATERAL_AMOUNT - 0.01) * COIN, obfuScationPool.collateralPubKey);
        tx.vin.push_back(vin);
        tx.vout.push_back(vout);
	else if CTxOut vout = CTxOut((MAX_COLLATERAL_AMOUNT - 0.01) * COIN, obfuScationPool.collateralPubKey);
        tx.vin.push_back(vin);
        tx.vout.push_back(vout);
        ...
        if (fAcceptable) {
            ...
            if (mi != mapBlockIndex.end() && (*mi).second) {
                ...
                if (pConfIndex->GetBlockTime() > sigTime) {
                    LogPrint("peernode","mnb - Bad sigTime %d for Masternode %s (%i conf block is at %d)\n",
            ...
            mn.Check(true);
            // add v11 peernodes, v12 should be added by mnb only
            if (protocolVersion < GETHEADERS_VERSION) {
                LogPrint("peernode", "dsee - Accepted OLD Masternode entry %i %i\n", count, current);
                Add(mn);
            }
            if (mn.IsEnabled()) {
                TRY_LOCK(cs_vNodes, lockNodes);
            ...
            LogPrint("peernode","dsee - Rejected Masternode entry %s\n", vin.prevout.hash.ToString());

            int nDoS = 0;
            if (state.IsInvalid(nDoS)) {
                LogPrint("peernode","dsee - %s from %i %s was not accepted into the memory pool\n", tx.GetHash().ToString().c_str(),
        ...
        //LogPrint("peernode","dseep - Received: vin: %s sigTime: %lld stop: %s\n", vin.ToString().c_str(), sigTime, stop ? "true" : "false");
        ...
        CMasternode* pmn = this->Find(vin);
        if (pmn != NULL && pmn->protocolVersion >= peernodePayments.GetMinMasternodePaymentsProto()) {
            // LogPrint("peernode","dseep - Found corresponding mn for vin: %s\n", vin.ToString().c_str());
            ...
            if (sigTime - pmn->nLastDseep > MASTERNODE_MIN_MNP_SECONDS) {
                ...
                if (!obfuScationSigner.VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, errorMessage)) {
                    LogPrint("peernode","dseep - Got bad Masternode address signature %s \n", vin.prevout.hash.ToString());
                    //Misbehaving(pfrom->GetId(), 100);
                    return;
                }

                // fake ping for v11 peernodes, ignore for v12
                ...
                if (pmn->IsEnabled()) {
                    TRY_LOCK(cs_vNodes, lockNodes);
                    if (!lockNodes) return;
                    LogPrint("peernode", "dseep - relaying %s \n", vin.prevout.hash.ToString());
                    BOOST_FOREACH (CNode* pnode, vNodes)
                        if (pnode->nVersion >= peernodePayments.GetMinMasternodePaymentsProto())
        ...
        LogPrint("peernode", "dseep - Couldn't find Masternode entry %s peer=%i\n", vin.prevout.hash.ToString(), pfrom->GetId());
...
void CMasternodeMan::Remove(CTxIn vin)
{
    ...
    while (it != vMasternodes.end()) {
        if ((*it).vin == vin) {
            LogPrint("peernode", "CMasternodeMan: Removing Masternode %s - %i now\n", (*it).vin.prevout.hash.ToString(), size() - 1);
...
void CMasternodeMan::UpdateMasternodeList(CMasternodeBroadcast mnb)
{
    ...
    LogPrint("peernode","CMasternodeMan::UpdateMasternodeList() -- peernode=%s\n", mnb.vin.prevout.ToString());
