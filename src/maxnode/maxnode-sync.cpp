...
{
    if (maxnodePayments.mapMasternodePayeeVotes.count(hash)) {
...
{
    switch (maxnodeSync.RequestedMasternodeAssets) {
    case MASTERNODE_SYNC_INITIAL:
...
void CMasternodeSync::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (strCommand == "ssc") { //Sync status count
        ...
        LogPrint("maxnode", "CMasternodeSync:ProcessMessage - ssc - got inventory count %d %d\n", nItemID, nCount);
    }
}
...
void CMasternodeSync::Process()
{
    ...
    LogPrint("maxnode", "CMasternodeSync::Process() - tick %d RequestedMasternodeAssets %d\n", tick, RequestedMasternodeAssets);
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        if (Params().NetworkID() == CBaseChainParams::REGTEST) {
            if (RequestedMasternodeAttempt <= 2) {
                ...
                uint256 n = 0;
                pnode->PushMessage("mnvs", n); //sync maxnode votes
            } else {
                RequestedMasternodeAssets = MASTERNODE_SYNC_FINISHED;
            }
        ...
        if (pnode->nVersion >= maxnodePayments.GetMinMasternodePaymentsProto()) {
            if (RequestedMasternodeAssets == MASTERNODE_SYNC_LIST) {
                LogPrint("maxnode", "CMasternodeSync::Process() - lastMasternodeList %lld (GetTime() - MASTERNODE_SYNC_TIMEOUT) %lld\n", lastMasternodeList, GetTime() - MASTERNODE_SYNC_TIMEOUT);
