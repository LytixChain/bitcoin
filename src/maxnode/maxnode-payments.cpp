...
CMasternodePayments maxnodePayments;
...
bool CMasternodePaymentDB::Write(const CMasternodePayments& objToSave)
{
    ...
    LogPrint("maxnode","Written info to mnpayments.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodePaymentDB::ReadResult CMasternodePaymentDB::Read(CMasternodePayments& objToLoad, bool fDryRun)
{
    ...
    LogPrint("maxnode","Loaded info from mnpayments.dat  %dms\n", GetTimeMillis() - nStart);
    LogPrint("maxnode","  %s\n", objToLoad.ToString());
    if (!fDryRun) {
        LogPrint("maxnode","Masternode payments manager - cleaning....\n");
        objToLoad.CleanPaymentList();
        LogPrint("maxnode","Masternode payments manager - result:\n");
        LogPrint("maxnode","  %s\n", objToLoad.ToString());
    }
...
void DumpMasternodePayments()
{
    ...
    LogPrint("maxnode","Verifying mnpayments.dat format...\n");
    ...
    if (readResult == CMasternodePaymentDB::FileError)
        LogPrint("maxnode","Missing budgets file - mnpayments.dat, will try to recreate\n");
    else if (readResult != CMasternodePaymentDB::Ok) {
        LogPrint("maxnode","Error reading mnpayments.dat: ");
    ...
    LogPrint("maxnode","Writting info to mnpayments.dat...\n");
...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    if (maxnodePayments.IsTransactionValid(txNew, nBlockHeight))
        return true;
    LogPrint("maxnode","Invalid mn payment detected %s\n", txNew.ToString().c_str());

    if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT))
        return false;
    LogPrint("maxnode","Masternode payment enforcement is disabled, accepting block\n");
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZPIVStake)
{
    ...
    CAmount maxnodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZPIVStake);
...
void CMasternodePayments::ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnget") { //Masternode Payments Request Sync
        ...
        if (maxnodePayments.mapMasternodePayeeVotes.count(winner.GetHash())) {
            LogPrint("mnpayments", "mnw - Already seen - %s bestHeight %d\n", winner.GetHash().ToString().c_str(), nHeight);
            maxnodeSync.AddedMasternodeWinner(winner.GetHash());
            return;
        }
        ...
        if (!winner.IsValid(pfrom, strError)) {
            // if(strError != "") LogPrint("maxnode","mnw - invalid message - %s\n", strError);
            return;
        }

        if (!maxnodePayments.CanVote(winner.vinMasternode.prevout, winner.nBlockHeight)) {
            //  LogPrint("maxnode","mnw - maxnode already voted - %s\n", winner.vinMasternode.prevout.ToStringShort());
            return;
        }
        ...
        if (maxnodePayments.AddWinningMasternode(winner)) {
            winner.Relay();
            maxnodeSync.AddedMasternodeWinner(winner.GetHash());
        }
    }
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
        ...
        BOOST_FOREACH (CTxOut out, txNew.vout) {
            if (payee.scriptPubKey == out.scriptPubKey) {
                if(out.nValue >= requiredMasternodePayment)
                    ...
                    LogPrint("maxnode","Masternode payment is out of drift range. Paid=%s Min=%s\n", FormatMoney(out.nValue).c_str(), FormatMoney(requiredMasternodePayment).c_str());
            }
        }
    ...
    LogPrint("maxnode","CMasternodePayments::IsTransactionValid - Missing required payment of %s to %s\n", FormatMoney(requiredMasternodePayment).c_str(), strPayeesPossible.c_str());
    return false;
}
...
void CMasternodePayments::CleanPaymentList()
{
    ...
    while (it != mapMasternodePayeeVotes.end()) {
        ...
        if (nHeight - winner.nBlockHeight > nLimit) {
            ...
            maxnodeSync.mapSeenSyncMNW.erase((*it).first);
...
bool CMasternodePaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (!pmn) {
        ...
        LogPrint("maxnode","CMasternodePaymentWinner::IsValid - %s\n", strError);
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    if (budget.IsBudgetPaymentBlock(nBlockHeight)) {
        ...
        LogPrint("maxnode","CMasternodePayments::ProcessBlock() Start nHeight %d - vin %s. \n", nBlockHeight, activeMasternode.vin.prevout.hash.ToString());
        ...
        if (pmn != NULL) {
            LogPrint("maxnode","CMasternodePayments::ProcessBlock() Found by FindOldestNotInVec \n");
            ...
            LogPrint("maxnode","CMasternodePayments::ProcessBlock() Winner payee %s nHeight %d. \n", address2.ToString().c_str(), newWinner.nBlockHeight);
        } else {
            LogPrint("maxnode","CMasternodePayments::ProcessBlock() Failed to find maxnode to pay\n");
        }
    }
    ...
    if (!obfuScationSigner.SetKey(strMasterNodePrivKey, errorMessage, keyMasternode, pubKeyMasternode)) {
        LogPrint("maxnode","CMasternodePayments::ProcessBlock() - Error upon calling SetKey: %s\n", errorMessage.c_str());
        return false;
    }

    LogPrint("maxnode","CMasternodePayments::ProcessBlock() - Signing Winner\n");
    if (newWinner.Sign(keyMasternode, pubKeyMasternode)) {
        LogPrint("maxnode","CMasternodePayments::ProcessBlock() - AddWinningMasternode\n");
