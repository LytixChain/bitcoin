...
int64_t CMasternode::GetLastPaid()
{
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
        ...
        if (maxnodePayments.mapMasternodeBlocks.count(BlockReading->nHeight)) {
            /*
                Search for this payee, with at least 2 votes. This will aid in consensus allowing the network
                to converge on the same payees quickly, then keep the same schedule.
            */
            if (maxnodePayments.mapMasternodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2)) {
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, bool fOffline)
{
    ...
    if (!fOffline && !maxnodeSync.IsBlockchainSynced()) {
        ...
        LogPrint("maxnode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }

    if (!obfuScationSigner.GetKeysFromSecret(strKeyMasternode, keyMasternodeNew, pubKeyMasternodeNew)) {
        strErrorRet = strprintf("Invalid maxnode key %s", strKeyMasternode);
        LogPrint("maxnode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }
...
bool CMasternodeBroadcast::Create(CTxIn txin, CService service, CKey keyCollateralAddressNew, CPubKey pubKeyCollateralAddressNew, CKey keyMasternodeNew, CPubKey pubKeyMasternodeNew, std::string& strErrorRet, CMasternodeBroadcast& mnbRet)
{
    ...
    LogPrint("maxnode", "CMasternodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyMasternodeNew.GetID() = %s\n",
    ...
    if (!mnbRet.IsValidNetAddr()) {
        strErrorRet = strprintf("Invalid IP address %s, maxnode=%s", mnbRet.addr.ToStringIP (), txin.prevout.hash.ToString());
        LogPrint("maxnode","CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        mnbRet = CMasternodeBroadcast();
        return false;
    }
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrint("maxnode","mnb - Signature rejected, too far into the future %s\n", vin.prevout.hash.ToString());
        nDos = 1;
        return false;
    }
    ...
    if (protocolVersion < maxnodePayments.GetMinMasternodePaymentsProto()) {
        LogPrint("maxnode","mnb - ignoring outdated Masternode %s protocol version %d\n", vin.prevout.hash.ToString(), protocolVersion);
        return false;
    }
    ...
    if (pubkeyScript.size() != 25) {
        LogPrint("maxnode","mnb - pubkey the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        LogPrint("maxnode","mnb - pubkey2 the wrong size\n");
        nDos = 100;
        return false;
    }

    if (!vin.scriptSig.empty()) {
        LogPrint("maxnode","mnb - Ignore Not Empty ScriptSig %s\n", vin.prevout.hash.ToString());
        return false;
    }
    ...
    if (pmn->pubKeyCollateralAddress == pubKeyCollateralAddress && !pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS)) {
        ...
        LogPrint("maxnode","mnb - Got updated entry for %s\n", vin.prevout.hash.ToString());
        if (pmn->UpdateFromNewBroadcast((*this))) {
            pmn->Check();
            if (pmn->IsEnabled()) Relay();
        }
        maxnodeSync.AddedMasternodeList(GetHash());
    }
...
{
    // we are a maxnode with the same vin (i.e. already activated) and this mnb is ours (matches our Masternode privkey)
...

    {
        ...
        if (!lockMain) {
            ...
            maxnodeSync.mapSeenSyncMNB.erase(GetHash());
            return false;
        }
    ...
    LogPrint("maxnode", "mnb - Accepted Masternode entry\n");

    if (GetInputAge(vin) < MASTERNODE_MIN_CONFIRMATIONS) {
        LogPrint("maxnode","mnb - Input must have at least %d confirmations\n", MASTERNODE_MIN_CONFIRMATIONS);
    ...
    LogPrint("maxnode","mnb - Got NEW Masternode entry - %s - %lli \n", vin.prevout.hash.ToString(), sigTime);
    CMasternode mn(*this);
    mnodeman.Add(mn);
...
bool CMasternodePing::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, vchSig, keyMasternode)) {
        LogPrint("maxnode","CMasternodePing::Sign() - Error: %s\n", errorMessage);
        return false;
    }
...
{
    if (sigTime > GetAdjustedTime() + 60 * 60) {
        LogPrint("maxnode","CMasternodePing::CheckAndUpdate - Signature rejected, too far into the future %s\n", vin.prevout.hash.ToString());
        nDos = 1;
        return false;
    }

    if (sigTime <= GetAdjustedTime() - 60 * 60) {
        LogPrint("maxnode","CMasternodePing::CheckAndUpdate - Signature rejected, too far into the past %s - %d %d \n", vin.prevout.hash.ToString(), sigTime, GetAdjustedTime());
        nDos = 1;
        return false;
    }
    ...
    LogPrint("maxnode", "CMasternodePing::CheckAndUpdate - New Ping - %s - %s - %lli\n", GetHash().ToString(), blockHash.ToString(), sigTime);
    ...
    if (pmn != NULL && pmn->protocolVersion >= maxnodePayments.GetMinMasternodePaymentsProto()) {
        ...
        // LogPrint("maxnode","mnping - Found corresponding mn for vin: %s\n", vin.ToString());
        ...
        if (!pmn->IsPingedWithin(MASTERNODE_MIN_MNP_SECONDS - 60, sigTime)) {
            ...
            if (mi != mapBlockIndex.end() && (*mi).second) {
                if ((*mi).second->nHeight < chainActive.Height() - 24) {
                    LogPrint("maxnode","CMasternodePing::CheckAndUpdate - Masternode %s block hash %s is too old\n", vin.prevout.hash.ToString(), blockHash.ToString());
                ...
                if (fDebug) LogPrint("maxnode","CMasternodePing::CheckAndUpdate - Masternode %s block hash %s is unknown\n", vin.prevout.hash.ToString(), blockHash.ToString());
            ...
            LogPrint("maxnode", "CMasternodePing::CheckAndUpdate - Masternode ping accepted, vin: %s\n", vin.prevout.hash.ToString());

            Relay();
            return true;
        }
        LogPrint("maxnode", "CMasternodePing::CheckAndUpdate - Masternode ping arrived too early, vin: %s\n", vin.prevout.hash.ToString());
        //nDos = 1; //disable, this is happening frequently and causing banned peers
        return false;
    }
    LogPrint("maxnode", "CMasternodePing::CheckAndUpdate - Couldn't find compatible Masternode entry, vin: %s\n", vin.prevout.hash.ToString());
