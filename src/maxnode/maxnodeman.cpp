...
bool CMasternodeDB::Write(const CMasternodeMan& mnodemanToSave)
{
    ...
    ssMasternodes << strMagicMessage;                   // maxnode cache file specific magic message
    ...
    LogPrint("maxnode","Written info to mncache.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodeDB::ReadResult CMasternodeDB::Read(CMasternodeMan& mnodemanToLoad, bool fDryRun)
{
    ...
    LogPrint("maxnode","Loaded info from mncache.dat  %dms\n", GetTimeMillis() - nStart);
    LogPrint("maxnode","  %s\n", mnodemanToLoad.ToString());
    if (!fDryRun) {
        LogPrint("maxnode","Masternode manager - cleaning....\n");
        mnodemanToLoad.CheckAndRemove(true);
        LogPrint("maxnode","Masternode manager - result:\n");
        LogPrint("maxnode","  %s\n", mnodemanToLoad.ToString());
    }
...
void DumpMasternodes()
{
    ...
    LogPrint("maxnode","Verifying mncache.dat format...\n");
    CMasternodeDB::ReadResult readResult = mndb.Read(tempMnodeman, true);
    // there was an error and it was not an error on file opening => do not proceed
    if (readResult == CMasternodeDB::FileError)
        LogPrint("maxnode","Missing maxnode cache file - mncache.dat, will try to recreate\n");
    else if (readResult != CMasternodeDB::Ok) {
        LogPrint("maxnode","Error reading mncache.dat: ");
    ...
    LogPrint("maxnode","Writting info to mncache.dat...\n");
    mndb.Write(mnodeman);

    LogPrint("maxnode","Masternode dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
bool CMasternodeMan::Add(CMasternode& mn)
{
    ...
    if (pmn == NULL) {
        LogPrint("maxnode", "CMasternodeMan: Adding new Masternode %s - %i now\n", mn.vin.prevout.hash.ToString(), size() + 1);
...
void CMasternodeMan::AskForMN(CNode* pnode, CTxIn& vin)
{
    ...
    LogPrint("maxnode", "CMasternodeMan::AskForMN - Asking node for missing entry, vin: %s\n", vin.prevout.hash.ToString());
...
void CMasternodeMan::CheckAndRemove(bool forceExpiredRemoval)
{
    ...
    while (it != vMasternodes.end()) {
        if ((*it).activeState == CMasternode::MASTERNODE_REMOVE ||
            ...
            (*it).protocolVersion < maxnodePayments.GetMinMasternodePaymentsProto()) {
            LogPrint("maxnode", "CMasternodeMan: Removing inactive Masternode %s - %i now\n", (*it).vin.prevout.hash.ToString(), size() - 1);
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        ...
        if (IsSporkActive (SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
            ...
            if ((nMasternode_Age) < nMasternode_Min_Age) {
                continue; // Skip maxnodes younger than (default) 8000 sec (MUST be > MASTERNODE_REMOVAL_SECONDS)
            }
        }
        mn.Check ();
        if (!mn.IsEnabled ())
...
{
    int i = 0;
    protocolVersion = protocolVersion == -1 ? maxnodePayments.GetMinMasternodePaymentsProto() : protocolVersion;
...
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCount)
{
    ...
    */
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        ...
        if (mn.protocolVersion < maxnodePayments.GetMinMasternodePaymentsProto()) continue;

        //it's in the list (up to 8 entries ahead of current block to allow propagation) -- so let's skip it
        if (maxnodePayments.IsScheduled(mn, nBlockHeight)) continue;
...
CMasternode* CMasternodeMan::FindRandomNotInVec(std::vector<CTxIn>& vecToExclude, int protocolVersion)
{
    ...
    LogPrint("maxnode", "CMasternodeMan::FindRandomNotInVec - nCountEnabled - vecToExclude.size() %d\n", nCountEnabled - vecToExclude.size());
    if (nCountEnabled - vecToExclude.size() < 1) return NULL;

    int rand = GetRandInt(nCountEnabled - vecToExclude.size());
    LogPrint("maxnode", "CMasternodeMan::FindRandomNotInVec - rand %d\n", rand);
    bool found;
...
int CMasternodeMan::GetMasternodeRank(const CTxIn& vin, int64_t nBlockHeight, int minProtocol, bool fOnlyActive)
{
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        if (mn.protocolVersion < minProtocol) {
            LogPrint("maxnode","Skipping Masternode with obsolete version %d\n", mn.protocolVersion);
            continue;                                                       // Skip obsolete versions
        }
        ...
        if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
            nMasternode_Age = GetAdjustedTime() - mn.sigTime;
            if ((nMasternode_Age) < nMasternode_Min_Age) {
                if (fDebug) LogPrint("maxnode","Skipping just activated Masternode. Age: %ld\n", nMasternode_Age);
...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        if (pnode->fObfuScationMaster) {
            ...
            LogPrint("maxnode","Closing Masternode connection peer=%i \n", pnode->GetId());
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (mapSeenMasternodeBroadcast.count(mnb.GetHash())) { //seen
            maxnodeSync.AddedMasternodeList(mnb.GetHash());
            return;
        }
        ...
        if (mnb.CheckInputsAndAdd(nDoS)) {
            ...
            LogPrint("maxnode","mnb - Rejected Masternode entry %s\n", mnb.vin.prevout.hash.ToString());
        ...
        LogPrint("maxnode", "mnp - Masternode ping, vin: %s\n", mnp.vin.prevout.hash.ToString());
        ...
        BOOST_FOREACH (CMasternode& mn, vMasternodes) {
            ...
            if (mn.IsEnabled()) {
                LogPrint("maxnode", "dseg - Sending Masternode entry - %s \n", mn.vin.prevout.hash.ToString());
                if (vin == CTxIn() || vin == mn.vin) {
                    ...
                    if (vin == mn.vin) {
                        LogPrint("maxnode", "dseg - Sent 1 Masternode entry to peer %i\n", pfrom->GetId());
                        return;
                    }
                }
            }
        ...
        if (vin == CTxIn()) {
            ...
            LogPrint("maxnode", "dseg - Sent %d Masternode entries to peer %i\n", nInvCount, pfrom->GetId());
        }
    }
    /*
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (protocolVersion < maxnodePayments.GetMinMasternodePaymentsProto()) {
            LogPrintf("CMasternodeMan::ProcessMessage() : dsee - ignoring outdated Masternode %s protocol version %d < %d\n", vin.prevout.hash.ToString(), protocolVersion, maxnodePayments.GetMinMasternodePaymentsProto());
            Misbehaving(pfrom->GetId(), 1);
            return;
        }
            ...
            if (count == -1 && pmn->pubKeyCollateralAddress == pubkey && (GetAdjustedTime() - pmn->nLastDsee > MASTERNODE_MIN_MNB_SECONDS)) {
                ...
                if (pmn->nLastDsee < sigTime) { //take the newest entry
                    ...
                    if (pmn->IsEnabled()) {
                        ...
                        if (!lockNodes) return;
                        BOOST_FOREACH (CNode* pnode, vNodes)
                            if (pnode->nVersion >= maxnodePayments.GetMinMasternodePaymentsProto())
        ...
        LogPrint("maxnode", "dsee - Got NEW OLD Masternode entry %s\n", vin.prevout.hash.ToString());
        ...
        if (fAcceptable) {
            ...
            if (mi != mapBlockIndex.end() && (*mi).second) {
                ...
                if (pConfIndex->GetBlockTime() > sigTime) {
                    LogPrint("maxnode","mnb - Bad sigTime %d for Masternode %s (%i conf block is at %d)\n",
            ...
            mn.Check(true);
            // add v11 maxnodes, v12 should be added by mnb only
            if (protocolVersion < GETHEADERS_VERSION) {
                LogPrint("maxnode", "dsee - Accepted OLD Masternode entry %i %i\n", count, current);
                Add(mn);
            }
            if (mn.IsEnabled()) {
                TRY_LOCK(cs_vNodes, lockNodes);
            ...
            LogPrint("maxnode","dsee - Rejected Masternode entry %s\n", vin.prevout.hash.ToString());
        ...
        CMasternode* pmn = this->Find(vin);
        if (pmn != NULL && pmn->protocolVersion >= maxnodePayments.GetMinMasternodePaymentsProto()) {
            // LogPrint("maxnode","dseep - Found corresponding mn for vin: %s\n", vin.ToString().c_str());
            ...
            if (sigTime - pmn->nLastDseep > MASTERNODE_MIN_MNP_SECONDS) {
                ...
                if (!obfuScationSigner.VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, errorMessage)) {
                    LogPrint("maxnode","dseep - Got bad Masternode address signature %s \n", vin.prevout.hash.ToString());
                    //Misbehaving(pfrom->GetId(), 100);
                    return;
                }
        ...
        LogPrint("maxnode", "dseep - Couldn't find Masternode entry %s peer=%i\n", vin.prevout.hash.ToString(), pfrom->GetId());
...
void CMasternodeMan::Remove(CTxIn vin)
{
    ...
    while (it != vMasternodes.end()) {
        if ((*it).vin == vin) {
            LogPrint("maxnode", "CMasternodeMan: Removing Masternode %s - %i now\n", (*it).vin.prevout.hash.ToString(), size() - 1);
...
void CMasternodeMan::UpdateMasternodeList(CMasternodeBroadcast mnb)
{
    ...
    LogPrint("maxnode","CMasternodeMan::UpdateMasternodeList() -- maxnode=%s\n", mnb.vin.prevout.ToString());
